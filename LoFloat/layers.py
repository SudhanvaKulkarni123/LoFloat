...
class STERound(torch.autograd.Function):
    @staticmethod
    def forward(ctx, tensor, params, round_mode):
        return lof.virtual_round(tensor, params, round_mode=round_mode, stoch_len=0)
...
class LoF_Quantize(nn.Module):
    def __init__(self, params, round_mode=None):
        super().__init__()
        self.params = params
        self.round_mode = round_mode if round_mode is not None else lof.RoundingMode.RoundToNearestEven

    def __deepcopy__(self, memo):
        new = LoF_Quantize(self.params, self.round_mode)
        new.load_state_dict(self.state_dict())
        memo[id(self)] = new
        return new

    def forward(self, x):
        return STERound.apply(x, self.params, self.round_mode)  # STE: real quant forward, identity backward

    def extra_repr(self):
        return f'params={self.params}, round_mode={self.round_mode}'
    
def _quantize(self, tensor, params):
    if params is None:
        return tensor
    return STERound.apply(tensor, params, self.round_mode)
...
class LoF_Linear(nn.Module):
    ...
    def __init__(self, in_features, out_features,
        ...
        self.bias_ = nn.Parameter(torch.empty(out_features)) if bias else None
        nn.init.kaiming_uniform_(self.weight)
        if self.bias_ is not None:
            nn.init.zeros_(self.bias_)
    ...
    def forward(self, x):
        ...
        if self.bias_ is not None:
            b_q = self._quantize(self.bias_, self.bias_params)
            out = out + b_q
        return out
...
class LoF_Conv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        self.bias_ = nn.Parameter(torch.empty(out_channels)) if bias else None
        nn.init.kaiming_uniform_(self.weight)
        if self.bias_ is not None:
            nn.init.zeros_(self.bias_)
    ...
    def forward(self, x):
        x_q = self._quantize(x, self.act_params)
        w_q = self._quantize(self.weight, self.weight_params)
        bias_q = self._quantize(self.bias_, self.bias_params) if self.bias_ is not None else None
    ...
    def extra_repr(self):
        return (
            ...
            f'dilation={self.dilation}, groups={self.groups}, bias={self.bias_ is not None}, '
            f'act_mantissa={self.act_params.mantissa_bits}, '
            f'act_exponent={self.act_params.total_bits - self.act_params.mantissa_bits - 1}, '
            f'weight_mantissa={self.weight_params.mantissa_bits}, '
            f'weight_exponent={self.weight_params.total_bits - self.weight_params.mantissa_bits - 1}, '
